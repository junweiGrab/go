...
var (
	help = `Usage: ./login_gmail [fileDir]
...
func main() {
	...
	for index, f := range files {
		...
		go func(port int, fileName string) {
			defer wg.Done()
			do(fileName, port)
		}(port+index, fileDir+"/"+f.Name())
	}
	wg.Wait()
}

func start(port int, killed <-chan bool, done chan<- bool) {
	command := exec.Command("/home/ec2-user/login_gmail/geckodriver",
		fmt.Sprintf("--binary=/usr/bin/firefox"),
...
func do(fileName string, port int) {
	killed := make(chan bool)
	done := make(chan bool)
	// start a webdriver.
	go start(port, killed, done)
	<-done
	...
	// firefoxCapOptions := firefox.Capabilities{
	// 	Binary: browserPath,
	// 	// Args:   []string{"--headless", "--disable-gpu"},
	// 	// Args: []string{"--headless"},
	// }
	// caps.AddFirefox(firefoxCapOptions)
	...
	for _, email := range emails {
		statusID := login(email, "popolopo", port, caps)
		switch statusID {
		case PWDCHANGED:
...
func login(email, password string, port int, caps selenium.Capabilities) int {
